"""test_gate_suite.py — MinIO gate tests. Wave 14."""
import sys
import os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))
from ear_adapter_minio import MinIOEARAdapter, EARState
from gcg_analyzer import GCGAnalyzer
from gap_assertions import convergence_fingerprint, summary_stats


def test_gcg_01():
    adapter = MinIOEARAdapter(auth_enabled=False)
    fam = next(f for f in adapter.collect_operation_families() if f.name == "object_put")
...
def test_gcg_02():
    adapter = MinIOEARAdapter(auth_enabled=True, audit_log_enabled=False)
    report = GCGAnalyzer().analyze(adapter, target_system="MinIO")
    items = [a for a in report.assertions if a.operation_family == "object_put"]
...
def test_gcg_03():
    adapter = MinIOEARAdapter(auth_enabled=True, tls_enabled=True)
    fam = next(f for f in adapter.collect_operation_families() if f.name == "object_put")
    assert adapter.assess_ear_state(fam) == EARState.CRYSTALLIZED
    print("T-GCG-03 PASS: auth + TLS → CRYSTALLIZED (no AWS backstop, self-managed)")


def test_nd_01():
    a1, a2 = MinIOEARAdapter(), MinIOEARAdapter()
...
def test_nd_02():
    adapter = MinIOEARAdapter()
    fam = next(f for f in adapter.collect_operation_families() if f.name == "object_put")
    layers = [l.name for l in adapter.collect_governance_layers(fam)]
    for required in ["auth_required", "iam_policy"]:
        assert required in layers, f"T-ND-02 FAIL: {required} not in {layers}"
    print(f"T-ND-02 PASS: layers={layers}")


def test_nd_03():
    decl = MinIOEARAdapter().get_governance_declaration()
...
def test_ear_02():
    adapter = MinIOEARAdapter(auth_enabled=True)
    fam = next(f for f in adapter.collect_operation_families() if f.name == "object_put")
    assert adapter.assess_ear_state(fam) == EARState.CRYSTALLIZED
...
def test_ear_03():
    adapter = MinIOEARAdapter(auth_enabled=True, tls_enabled=True, audit_log_enabled=True, object_lock=True)
    ...
    print("T-EAR-03 PASS: no MinIO family reaches ACTIVE (no AWS security backstop)")
...
def compute_fingerprint():
    ...
    print("NOTE: CVE-2025-31489 signature bypass; CVE-2026-03-17 OIDC confusion; Evil MinIO active exploitation; no AWS backstop vs T1694")
    print("=" * 60)
    return fp
...
def run_all_gates():
    ...
    failed = 0
    failures = []
    print("Running " + str(len(tests)) + " gate tests (" + "MinIO" + ")...")
    for t in tests:
        try:
            t()
            passed += 1
